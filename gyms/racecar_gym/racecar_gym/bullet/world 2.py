...
from racecar_gym.bullet.positioning import (
    AutomaticGridStrategy,
    RandomPositioningStrategy,
...
class World(world.World):
    ...
    def get_starting_position(self, agent: Agent, mode: str) -> Pose:
        ...
        if mode == "grid":
            strategy = AutomaticGridStrategy(
                obstacle_map=self._maps["obstacle"], number_of_agents=len(self._agents)
            )
        elif mode == "random":
            ...
            if (
                start_index == 0
            ):  # on first agent, compute a fixed interval for sampling and copy occupancy map
                progresses = progress_map[
                    obstacle_map > min_distance_to_wall
                ]  # center has enough distance from the walls
                progresses = progresses[
                    (progresses > progress_radius)
                    & (progresses < (1 - progress_radius))
                ]  # center+-radius in [0,1]
                self._progress_center = np.random.choice(progresses)
                self._tmp_occupancy_map = self._maps["occupancy"].map.copy()
            strategy = RandomPositioningWithinBallStrategy(
                progress_map=self._maps["progress"],
                obstacle_map=self._maps["obstacle"],
        ...
        if mode == "random_ball":  # mark surrounding pixels as occupied
            ...
            neigh_sz = int(
                1.0 / self._maps["obstacle"].resolution
            )  # mark 1 meter around the car
            self._tmp_occupancy_map[
                px - neigh_sz : px + neigh_sz, py - neigh_sz : py + neigh_sz
            ] = False
    ...
    def _update_race_info(self, agent):
        ...
        if "checkpoint" in self._state[agent.id]:
            ...
            if last_checkpoint + 1 == checkpoint:
                ...
                last_checkpoint == self._config.map_config.checkpoints
                and checkpoint == 0
            ):
                ...
                last_checkpoint == 0
                and checkpoint == self._config.map_config.checkpoints
            ):
                self._state[agent.id]["wrong_way"] = True
        else:
            self._state[agent.id]["checkpoint"] = checkpoint
    ...
    def _update_ranks(self):
        agents = [
            ...
            for agent_id in map(lambda a: a.id, self._agents)
        ]

        ranked = [
            item[0]
            for item in sorted(
                agents, key=lambda item: (item[1], item[2]), reverse=True
            )
        ]
