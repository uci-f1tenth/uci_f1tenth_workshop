...
class SingleAgentRaceEnv(gymnasium.Env):
    ...
    def step(
        ...
        observation["time"] = np.array(
            state[self._scenario.agent.id]["time"], dtype=np.float32
        )
    ...
    def reset(
        ...
        obs = self._scenario.agent.reset(
            self._scenario.world.get_starting_position(self._scenario.agent, mode)
        )
        self._scenario.world.update()
        state = self._scenario.world.state()
    ...
    def render(self) -> Union[RenderFrame, list[RenderFrame], None]:
        if self._render_mode == "human":
            ...
            return self._scenario.world.render(
                mode=mode, agent_id=self._scenario.agent.id, **self._render_options
            )
