...
class MultiAgentScenario:
    ...
    def from_spec(path: str, rendering: bool = None) -> "MultiAgentScenario":
        ...
        agents = dict(
            [
                (
                    ...
                    Agent(
                        id=s.id,
                        vehicle=load_vehicle(s.vehicle),
                        task=task_from_spec(s.task),
                    ),
                )
        ...
        return MultiAgentScenario(
            world=load_world(spec.world, agents=list(agents.values())), agents=agents
        )
...
class SingleAgentScenario:
    ...
    def from_spec(path: str, rendering: bool = None) -> "SingleAgentScenario":
        ...
        agent = Agent(
            id=agent_spec.id,
            vehicle=load_vehicle(agent_spec.vehicle),
            task=task_from_spec(agent_spec.task),
        )

        return SingleAgentScenario(
            world=load_world(spec.world, agents=[agent]), agent=agent
        )
