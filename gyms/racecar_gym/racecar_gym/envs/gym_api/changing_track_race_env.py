...
class ChangingTrackRaceEnv(gymnasium.Env):
    ...
    def __init__(
        self,
        env_factories: List[Callable[[], gymnasium.Env]],
        order: str = "sequential",
        render_mode: str = "follow",
    ):
        super().__init__()
        self._current_track_index = 0
        self._render_mode = render_mode
        if order == "sequential":
            self._order_fn = lambda: (self._current_track_index + 1) % len(
                env_factories
            )
        elif order == "random":
        ...
        self._envs = [
            SubprocessEnv(factory=factory, blocking=True) for factory in env_factories
        ]
        assert all(self._envs[0].action_space == env.action_space for env in self._envs)
        assert all(
            self._envs[0].observation_space == env.observation_space
