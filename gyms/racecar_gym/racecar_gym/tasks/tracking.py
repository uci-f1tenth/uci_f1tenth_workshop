...
class WaypointFollow(Task):
    ...
    def reward(self, agent_id, state, action) -> float:
        ...
        delta_act = np.array(list(action.values())) - np.array(
            list(self._last_action.values())
        )
        cost = np.matmul(delta_pos, np.matmul(Q, delta_pos)) + np.matmul(
            delta_act, np.matmul(R, delta_act)
        )
        reward = np.exp(-cost)
    ...
    def _check_collision(self, agent_state):
        safe_margin = 0.25
        collision = (
            agent_state["wall_collision"] or len(agent_state["opponent_collisions"]) > 0
        )
