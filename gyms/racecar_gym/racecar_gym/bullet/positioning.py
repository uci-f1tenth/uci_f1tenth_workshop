...
class AutomaticGridStrategy(PositioningStrategy):
    ...
    def get_pose(self, agent_index: int) -> Pose:
        ...
        center_position = self._distance_map.to_meter(
            px + max_index[0], py + max_index[1]
        )
        if agent_index % 2 == 0:
...
class RandomPositioningStrategy(PositioningStrategy):
    def __init__(
        self,
        progress_map: GridMap,
        obstacle_map: GridMap,
        min_distance_to_obstacle: float = 0.5,
    ...
    def get_pose(self, agent_index: int) -> Pose:
        ...
        x, y, angle = self._random_position(
            self._progress, center_corridor, delta_progress_next_pos
        )
        return (x, y, 0.05), (0, 0, angle)

    def _random_position(
        self, progress_map, sampling_map, delta_progress_next_pos=0.025
    ):
        ...
        diff = np.array(progress_map.to_meter(npx, npy)) - np.array(
            progress_map.to_meter(px, py)
        )
...
class RandomPositioningWithinBallStrategy(RandomPositioningStrategy):
    def __init__(
        ...
        min_distance_to_obstacle: float = 0.5,
        progress_center: float = 0.0,
        progress_radius: float = 0.01,
    ):
        self._progress = progress_map
        self._obstacles = obstacle_map
        self._occupancy = drivable_map
        self._obstacle_margin = min_distance_to_obstacle
        self._progress_center = (
            progress_center  # the interval is centered w.r.t. a fixed progress value
        )
        self._progress_radius = (
            progress_radius  # the width of the interval is +-progress_radius
        )

    def get_pose(self, agent_index: int) -> Pose:
        progress_interval = np.logical_and(
            self._progress.map >= self._progress_center - self._progress_radius,
            self._progress.map <= self._progress_center + self._progress_radius,
        )
        center_corridor = np.argwhere(
            np.logical_and(
                progress_interval,
