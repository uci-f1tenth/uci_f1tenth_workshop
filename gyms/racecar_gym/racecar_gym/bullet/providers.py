...
from racecar_gym.bullet.configs import (
    SensorConfig,
    VehicleConfig,
    ActuatorConfig,
    SceneConfig,
)
from racecar_gym.bullet.sensors import (
    Lidar,
    PoseSensor,
    AccelerationSensor,
    VelocitySensor,
...
def load_sensor(config: SensorConfig) -> BulletSensor:
    if config.type == "lidar":
        return Lidar(
            name=config.name, type=config.type, config=Lidar.Config(**config.params)
        )
    if config.type == "pose":
        return PoseSensor(
            name=config.name,
            type=config.type,
            config=PoseSensor.Config(**config.params),
        )
    if config.type == "acceleration":
        return AccelerationSensor(
            name=config.name,
            type=config.type,
            config=AccelerationSensor.Config(**config.params),
        )
    if config.type == "velocity":
        return VelocitySensor(
            name=config.name,
            type=config.type,
            config=VelocitySensor.Config(**config.params),
        )
    if config.type == "rgb_camera":
        return RGBCamera(
            name=config.name, type=config.type, config=RGBCamera.Config(**config.params)
        )
...
def load_actuator(config: ActuatorConfig) -> BulletActuator:
    ...
    if config.type == "steering":
        return SteeringWheel(
            name=config.name, config=SteeringWheel.Config(**config.params)
        )
...
def load_vehicle(spec: VehicleSpec) -> core.Vehicle:
    ...
    sensors = [
        FixedTimestepSensor(
            sensor=load_sensor(config=c), frequency=c.frequency, time_step=0.01
        )
        for c in sensors
    ]
    ...
    if not requested_actuators.issubset(available_actuators):
        raise NotImplementedError(
            f"Actuators {requested_actuators - available_actuators} not available."
        )
    ...
    car_config = RaceCar.Config(
        urdf_file=config.urdf_file, color=_compute_color(config.color)
    )
...
def load_world(spec: WorldSpec, agents: List[Agent]) -> core.World:
    ...
    config.map.starting_grid = resolve_path(
        file=config_file, relative_path=config.map.starting_grid
    )
