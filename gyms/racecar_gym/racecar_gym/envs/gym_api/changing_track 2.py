...
class ChangingTrackMultiAgentRaceEnv(ChangingTrackRaceEnv):
    ...
    def __init__(
        ...
        env_factories = [
            self._make_factory(
                scenario=s, render_mode=render_mode, render_options=render_options
            )
            for s in scenarios
        ]
    ...
    def _make_factory(
        self, scenario: str, render_mode: str, render_options: Dict
    ) -> Callable[[], MultiAgentRaceEnv]:
        def factory():
            return MultiAgentRaceEnv(
                scenario=scenario,
...
class ChangingTrackSingleAgentRaceEnv(ChangingTrackRaceEnv):
    ...
    def _make_factory(
        self, scenario: str, render_mode: str, render_options: Dict
    ) -> Callable[[], SingleAgentRaceEnv]:
        def factory():
            return SingleAgentRaceEnv(
                scenario=scenario,
                render_mode=render_mode,
                render_options=render_options,
            )
