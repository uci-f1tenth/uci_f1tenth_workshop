...
class Agent():
    def __init__(self):
        # define tools
        # TODO: tune the constants to follow our car.
        self.const = Constants()
        self.pid_controller = PID()

        # tuning variables(check constants.py for constants)
        self.pid_controller.change_tuning_parameters(kp=self.const.KP, ki=self.const.KI, kd=self.const.KD) # tune PID
    ...
    def get_range(self, range_data, angle):
        ...
        # print(index)
        if not np.isnan(range_data[index]) and not np.isinf(range_data[index]):
